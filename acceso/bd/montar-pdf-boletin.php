...
if ($nombre_archivo){
	if (!((strpos($tipo_archivo, "pdf")) && ($tamano_archivo < 6000000))) { 
		...
		if (copy($_FILES['pdf']['tmp_name'], "$uploads_dir/$new_name")){ 
			//			copy($_FILES['pdf']['tmp_name'], "imagenes/boletins/$new_name");
			$aviso ='El archivo ha sido cargado correctamente. ->';
			//ACTUALIZAR EL NOMBRE DEL pdf
			$usql_pdf=("UPDATE boletines SET pdf='".$new_name."' WHERE id_noticia='".$id_noticia."'");
			...
			$bsql="UPDATE boletines SET pdf='' WHERE id_noticia='".$id_noticia."'";
